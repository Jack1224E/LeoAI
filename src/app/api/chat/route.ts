export const maxDuration = 240; // Increased for Deepthink

import { spawn } from "child_process";

async function runCodex(systemPrompt: string, userPrompt: string, model: string): Promise<string> {
  return new Promise<string>((resolve, reject) => {
    // We combine the system prompt and user prompt since Codex exec takes a single prompt argument
    ...
    const child = spawn("npx", [
      ...
      env: {
        ...process.env,
        CODEX_GATEWAY_API_KEY: "sk-2K6SRMPbAk4QMHgn8FQJYz6dqdsvX8WhMCcinKqQnRhwdtWi",
      }
    });
    ...
    child.on("close", (code) => {
      ...
      if (output.includes(codexMarker)) {
        finalResponse = output.split(codexMarker)[1];
        if (finalResponse.includes(tokensMarker)) {
          finalResponse = finalResponse.split(tokensMarker)[0];
        }
      }
      
      if (code !== 0 && !output.includes(codexMarker)) {
        reject(new Error(`Codex CLI exited with code ${code}. Stderr: ${errOutput}`));
      } else {
...
export async function POST(req: Request) {
  try {
    const { messages, model, isDeepthink } = await req.json();
    ...
    const selectedModel = (model === "jlm" || !model) ? "deepseek-v4-flash" : model;
    const baseSystemPrompt = `You are Jack AI. Never mention files, directories, code editing, working directories, or developer tools. Adhere strictly to the following protocol for every response:
...
a single framework name or consulting word in the text? Fix whatever fails.`;
    ...
    if (!isDeepthink) {
      responseText = await runCodex(baseSystemPrompt, prompt, selectedModel);
    } else {
      const brainstormerSystem = "You are the Brainstormer. Explore all possible angles, weird edge cases, and divergent ideas for this prompt. Do not worry about constraints or reality yet, just brainstorm aggressively. Output a list of unconstrained ideas.";
      const criticSystem = "You are the Critic. Aggressively tear down the user's prompt. Find security flaws, physical constraints, logic holes, and scaling limits. Be brutal and pessimistic. Point out everything that could go wrong.";
      
      // Run Brainstormer and Critic in parallel
      const [brainstormOut, criticOut] = await Promise.all([
        runCodex(brainstormerSystem, prompt, selectedModel).catch(e => "Brainstormer failed: " + e.message),
        runCodex(criticSystem, prompt, selectedModel).catch(e => "Critic failed: " + e.message)
      ]);
      
      const synthesizerUserPrompt = `User Prompt:
${prompt}
...
---
Based on the User Prompt and the additional context from the Brainstormer and Critic above, write your final, definitive response using your strict operating engineer protocol.`;

      responseText = await runCodex(baseSystemPrompt, synthesizerUserPrompt, selectedModel);
    }
    ...
    return new Response(JSON.stringify({
      ...
      choices: [{
        index: 0,
        message: { role: "assistant", content: responseText || "No response generated." },
        finish_reason: "stop"
      }]
    }), {
      status: 200,
