...
const maxDuration = 240; // Increased for Deepthink
;
async function runClaude(systemPrompt, userPrompt, model) {
    return new Promise((resolve, reject)=>{
        const child = (0, __TURBOPACK__imported__module__$5b$externals$5d2f$child_process__$5b$external$5d$__$28$child_process$2c$__cjs$29$__["spawn"])("claude", [
            "--print",
            "--permission-mode",
            "dontAsk",
            "--model",
...
async function POST(req) {
    try {
        ...
        // Default to the original underlying model if jlm is selected, or use whatever is passed
        const selectedModel = model === "jlm" || !model ? "glm-5.3" : model;
...
a single framework name or consulting word in the text? Fix whatever fails.`;
        ...
        if (!isDeepthink) {
            ...
            const rawOut = await runClaude(baseSystemPrompt, prompt, selectedModel);
            responseText = rawOut.split("\n").filter((line)=>!line.startsWith("[claude-code:")).join("\n").trim();
        } else {
            // DEEPTHINK MODE: Parliament Architecture
            ...
            const [brainstormRaw, criticRaw] = await Promise.all([
                runClaude(brainstormerSystem, prompt, selectedModel).catch((e)=>"Brainstormer failed: " + e.message),
                runClaude(criticSystem, prompt, selectedModel).catch((e)=>"Critic failed: " + e.message)
            ]);
            const brainstormOut = brainstormRaw.split("\n").filter((line)=>!line.startsWith("[claude-code:")).join("\n");
            const criticOut = criticRaw.split("\n").filter((line)=>!line.startsWith("[claude-code:")).join("\n");
            // Now pass their output to the Synthesizer (Staff Engineer)
...
Based on the User Prompt and the additional context from the Brainstormer and Critic above, write your final, definitive response using your strict operating engineer protocol.`;
            const finalRaw = await runClaude(baseSystemPrompt, synthesizerUserPrompt, selectedModel);
            responseText = finalRaw.split("\n").filter((line)=>!line.startsWith("[claude-code:")).join("\n").trim();
        }
...
//# sourceMappingURL=%5Broot-of-the-server%5D__1kie32e._.js.map